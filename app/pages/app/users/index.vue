...
import type { IUserDocument } from "~~/server/models/users.model";
...
const {
  ...
  pending,
  refresh,
} = await useFetch<IUserDocument[]>("/api/users", {
  default: () => [],
});

const filtered = computed(() =>
  users.value.filter((c) =>
    c.name.toLowerCase().includes(search.value.toLowerCase()),
  ),
);

const deleteTarget = ref<IUserDocument | null>(null);

const resolveDeleteUrl = (user: Record<string, any>) =>
  `/api/users/${user._id}/delete`;

const handleDeleted = (deleted: Record<string, any>) => {
  users.value = users.value.filter((c) => c._id !== deleted._id);
};
</script>
...
<template>
  <ListWrapper
    title="Users"
    resource-label="User"
    description="Every user shown on the public site."
    create-to="/app/users/create"
